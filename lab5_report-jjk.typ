...
综合前面各节的推导结果，ZJU-I 型机械臂的几何雅可比矩阵可以表示为：
...
其中，$J_v in bb(R)^(3 times 6)$ 为线速度雅可比，$J_omega in bb(R)^(3 times 6)$ 为角速度雅可比。为简化表达，记 *$Sigma = q_2 + q_3 + q_4$*。
...
#align(center)[
  #text(size: 9pt, style: "italic")[
    注：上述雅可比矩阵表达式中所有三角函数参数均为关节变量 $q_1, q_2, dots, q_6$ 的组合，\
    完整描述了机械臂末端速度与关节速度之间的映射关系：$vec(v_e, omega_e) = J(q) dot(q)$
  ]
]
