...
1. 首先通过`Simulation->Simulation settings`进入仿真设置界面，通过修改`Gravity vector`为全0取消重力
...
#figure(
  image("images/gravity.png", width: 40%),
  caption: "修改Gravity vector取消重力",
)
...
#figure(
  grid(
    columns: 2,
    gutter: 0mm,
    figure(numbering: none, image("images/dynamicmode.png", width: 60%)),
    figure(numbering: none, image("images/velocity.png", width: 100%)),
  ),
  caption: "设置为速度控制模式",
)
...
Jacobian矩阵建立了机械臂*关节速度*与末端速度*末端速度*之间的线性映射关系：
#v(-0.5em)
$
...
+ *数值不稳定*：即使不完全奇异，$bold(J)$ 接近奇异时，直接求逆的数值误差也会被放大。

为了解决奇异性问题，采用*阻尼最小二乘法*（也称为Levenberg-Marquardt方法）：
#v(-0.5em)
$
...
#h(2em)速度方向始终与位置向量 $bold(p)(t) - bold(p)_c$ 垂直，即沿圆的*切线方向*。
...
圆锥运动是*纯姿态变化*的任务，末端点位置固定，仅姿态绕圆锥轴旋转。
...
+ 在机械臂轨迹规划中，涉及两种主要的坐标系：
  + *关节空间*：由各关节角度 $bold(q) = [theta_1, theta_2, theta_3, theta_4, theta_5, theta_6]^T$ 组成的空间。
  + *笛卡尔空间*：由末端执行器的位置和姿态 $bold(p) = [x, y, z, alpha, beta, gamma]^T$ 组成的空间。
...
$
#v(-0.5em)
#h(2em)其中 $R = 0.05$ m 为半径，$T$ 为运动周期。对于每个时间段，使用五次多项式在关节空间插值。

=== 圆锥轨迹规划
+ 圆锥运动的特点是末端点位置固定，但末端执行器的姿态按圆锥面旋转。设圆锥顶点为 $bold(p)_"apex"$，半锥角为 $alpha_"cone" = 30degree$。
+ 末端执行器的*轴线（approach vector）*在圆锥面上运动，可以用球坐标描述：
  #v(-0.5em)
  $
    bold(a)(phi) = mat(
...
+ 为了保证姿态的连续性，引入*自旋角（spin angle）参数*，通过优化算法选择使关节角变化最小的解。
...
= 总结
本实验通过速度控制和位置控制两种方法实现了ZJU-I型机械臂在笛卡尔空间的轨迹规划任务。实验结果表明，两种控制方式各有特点，均能成功完成正方形、圆形和圆锥三种轨迹的跟踪。

速度控制方法通过Jacobian矩阵建立关节速度与末端速度的直接映射关系，具有较好的实时性和直观性。该方法在圆形和正方形轨迹上表现良好，因为这两类任务的末端速度可以直接定义。然而，速度控制在圆锥任务中的实现相对简单：由于末端点位置固定，线速度为零，仅需给定恒定的角速度向量即可完成轨迹跟踪，无需复杂的姿态规划。
...
通过本次实验，我们深入理解了笛卡尔空间轨迹规划的原理，掌握了Jacobian矩阵的应用、五次多项式插值以及逆运动学多解选择等关键技术，为后续复杂机器人任务的开发奠定了坚实基础。