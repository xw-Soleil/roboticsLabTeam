...
== 实验结果
通过对ZJU-I型机械臂的控制，成功实现了机械臂按照预定轨迹绘制圆形、方形和圆锥的任务。具体结果如下：
#tip[
...
+ *机械臂圆形绘制任务*
  #grid(
    ...
    align: horizon,
    figure(
      image("images/lab6/开始绘制_圆.png", width: 4cm),
      caption: [机械臂绘制圆形过程1],
    ),
    figure(
      image("images/lab6/绘制过程_圆.png", width: 4cm),
      caption: [机械臂绘制圆形过程2],
    ),
    figure(
      image("images/lab6/goHome_圆.png", width: 3.8cm),
      caption: [机械臂归位],
    ),
  )
...
+ *机械臂正方形绘制任务*
  #grid(
    ...
    align: horizon,
    figure(
      image("images/lab6/开始绘制_方.png", width: 4cm),
      caption: [方形过程1],
    ),
    figure(
      image("images/lab6/绘制过程_方.png", width: 4cm),
      caption: [方形过程2],
    ),
    figure(
      image("images/lab6/绘制过程2_方.png", width: 3.97cm),
      caption: [方形过程3],
    ),
    figure(
      image("images/lab6/goHome_方.png", width: 3.7cm),
      caption: [机械臂归位],
    ),
  )
...
+ *机械臂圆锥轨迹绘制任务*
  #grid(
    ...
    align: horizon,
    figure(
      image("images/lab6/圆锥过程1.png", width: 4cm),
      caption: [圆锥绘制过程1],
    ),
    figure(
      image("images/lab6/圆锥过程2.png", width: 4cm),
      caption: [圆锥绘制过程2],
    ),
    figure(
      image("images/lab6/圆锥过程3.png", width: 4cm),
      caption: [圆锥绘制过程3],
    ),
    figure(
      image("images/lab6/圆锥过程4.png", width: 4.3cm),
      caption: [圆锥绘制过程4],
    )
  )
...
+ 对于*绘制圆形任务*，机械臂能够平滑地沿预定轨迹运动，圆形轮廓清晰，误差较小。
+ 在*绘制正方形*过程中，机械臂在转角处的速度控制较好，能够准确到达各个顶点位置，整体形状符合预期。
+ *圆锥绘制任务*中，机械臂能够实现从底部到顶部的平滑过渡，圆锥形状明显，显示出良好的轨迹控制能力。

#note[
  实验过程中，由于圆锥绘制轨迹角度变化较大，因此在经过不断坐标调整和程序优化后，机械臂才可在规定最大旋转角度范围之内顺利完成任务。
]

= 实验原理
由于实物机械臂的控制只支持位置控制接口，因此本实验主要通过位置控制方式实现轨迹跟踪。具体方法在实验五报告中已经有所阐述，此处不再赘述。

#pagebreak()


#part[实验6.2 机械臂抓取与搬运实验]
...
+ 第一个木块成功从起始位置搬运至目标区域，从A点运动到B点实现了直线运动。
  #grid(
    ...
    align: horizon,
    figure(
      image("images/lab6/抓取.png", width: 1.95cm),
      caption: [机械臂抓取动作],
    ),
    figure(
      image("images/lab6/straightA.png", width: 4cm),
      caption: [A点起始],
    ),
    figure(
      image("images/lab6/straight.png", width: 4cm),
      caption: [运动到B点],
    ),
    figure(
      image("images/lab6/放置1.png", width: 4.2cm),
      caption: [机械臂放置动作],
    ),
  )
