...
== 实验目的
1. 了解 ZJU-I 型机械臂的轨迹规划方法。
2. 熟悉 ZJU-I 型机械臂的逆运动学求解方法。
3. 学习对 ZJU-I 型机械臂的搬运任务进行目标点的选取和轨迹规划。
...
== 结果分析
+ *速度边界条件*：通过在笛卡尔空间规划速度并利用雅可比矩阵转换到关节空间，实现了直线段前后的平滑过渡。
+ *中间点策略*：在返回起始区域时设置中间过渡点，避免了关节角的剧烈变化和潜在碰撞的发生。
+ *逆运动学多解选择*：对于多解情况，选择与当前关节状态最接近且无奇异性的解，保证了运动的连续性。
+ *运动时间分配*：各段运动时间根据实际距离和速度限制合理分配，并根据实际实物机械臂进行调整。
...
#part[实验总结]
本次实验成功实现了机械臂的轨迹规划任务、机械臂的抓取与搬运任务，主要成果包括：
...
#v(-0.5em)
通过本次实验，我们深入理解了机器人轨迹规划的原理和方法，掌握了实际编程实现的技巧，为后续更复杂的机器人控制任务打下了坚实的基础。
